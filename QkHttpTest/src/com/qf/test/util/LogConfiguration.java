...
public class LogConfiguration {
	...
	static Logger logger = Logger.getLogger(LogConfiguration.class.getName());

	public static void initLogFromConfigFile(String logConfgFilePath)
			throws Exception {
		try {
			PropertyConfigurator.configure(logConfgFilePath);
		} catch (Exception ex) {
			logger.error("LogConfiguration.initLogFromConfigFile throw Exception: "
					+ ex.getMessage() + logConfgFilePath);
		}
	}
	...
	public static void initLog(String fileName) {
		...
		// 声明日志文件存储路径以及文件名、格式
		final String logFilePath = "./result/log/" + founctionName + "/"
				+ fileName + ".log";
